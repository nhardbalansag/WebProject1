...
<div id="mainAdminContents">
	<div id="content" class="content" style="padding-bottom: 70px;">
 			<div style="width: 95%; justify-content: flex-start; margin:auto;">
				<a href=
					...
					<?php 
						...
						echo "transactionControlPage.php?customerTransactionInformation=" . $_GET['viewOneCustomerTransactions']
 			...
 			<div style="text-align: center;">
 				<p style="font-size: 20px; font-weight: bolder;">transaction</p>
	 		</div>
			<div id="tableReport">
			</div>
 		</div>
...
<script type="text/javascript">
	var apiLinkget = '../../../../api/creator/admin/transaction/radAllOneCustomerTransaction.php';

	const tableReport = document.getElementById('tableReport');

 	var reference = window.referenceVar = '<?= $_GET['viewOneCustomerTransactions'] ?>';
	       ...
	       if (request.readyState === 4) {   // XMLHttpRequest.DONE == 4
	            ...
	            if (request.status === 200) {
	            	...
	            	if(res.response.http_response_code === 200 && res.response.reason === 'success'){
	            		...
	            		const headArray = ["date", "status", "payment", "current balance"];
	            		...
	            		for(var i = 0; i < count; i++){
	            			...
	            			var date = res.response.display.result[i].transaction.datePaided;
		            		var status = res.response.display.result[i].transaction.status;
		            		var payment = res.response.display.result[i].transaction.paidedAmount;
		            		var currentbalance = res.response.display.result[i].transaction.currentBalance;

		            		const dataArray = [date.split(" ")[0], status, payment, currentbalance];
