...
<div id="mainAdminContents">
	<div id="content" class="content" style="padding-bottom: 70px;">
 			...
 			<div style="text-align: center;">
 				<p style="font-size: 20px; font-weight: bolder;">edit penalty amount</p>
 				<p id="result"></p>
	 		</div>
	 		<div style="text-align: center;" id="hideInput">
	 			...
	 			<form method="post" id="formDataFile">
					...
					<div class="inputmobile adminInputDivs">
						...
						<select class="inputmobile" id="status">
 ...
 <script type="text/javascript">
 	...
 	const apiLinkedit = '../../../../api/creator/admin/transaction/editPenaltyAmount.php';
 	const apiLinkget = '../../../../api/creator/admin/transaction/readAllPenaltyAmount.php';
 	const apiLinkgetOne = '../../../../api/creator/admin/transaction/readOnePenaltyAmount.php';


 	var reference = window.ulrReference = '<?= $_GET['editPenaltyAmount']  ?>';
	...
	function AjaxCreateInput(tojson) {
	   ...
	   request.onreadystatechange = function() {
	       ...
	       if (request.readyState === 4) {   // XMLHttpRequest.DONE == 4
	            ...
	            if (request.status === 200) {
	            	...
	            	if(res.response.http_response_code === 200 && res.response.reason === 'success'){
	           			...
	           			pct_tittle.innerHTML = res.response.display.result[0].penalty_amount + " " + res.response.display.result[0].penalty_status
		...
		request.open("POST", apiLinkedit, true);
	...
	function AjaxdisplayOne(tojson) {
	   ...
	   request.onreadystatechange = function() {
	       ...
	       if (request.readyState === 4) {   // XMLHttpRequest.DONE == 4
	            ...
	            if (request.status === 200) {
	            	...
	            	if(res.response.http_response_code === 200 && res.response.reason === 'success'){
	            		...
	            		penalty.value = res.response.display.result[0].penalty_amount;
		...
		request.open("POST", apiLinkgetOne, true);
		request.setRequestHeader('Content-Type', 'Application/json');
		request.send(tojson);
	}// end of the function
