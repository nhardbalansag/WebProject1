<div id="content">
	...
	<div class="informations">
		...
		<div style="text-align: center; width: 80%; margin:auto">
				...
				<p id="inquiryResultlabel" style="font-size: 20px;">bernard messages</p>
			</div>
...
<script type="text/javascript">
	...
	const APIcustomerAccounts = '../../../api/creator/support/viewAllCustomerAccountMessages.php';
	...
	function getInfoId(){
		const customerAccount = window.reference = '<?= $_SESSION['customerAccountPersonalInformationID'] ?>';
		var data = {
			referenceInput:{
			...
			if (request.status == 200) {
				...
				if(res.response.http_response_code === 200 && res.response.reason === 'success'){
					...
					for(var i = 0; i <  res.response.result.data.length; i++){
						...
						date_p.innerHTML = res.response.result.data[i].createdMessage.dateCreated;
						accountName.innerHTML = res.response.result.data[i].personalInformation.ci_firstname;
						...
						a.href = 'support_controlPage.php?CustomerAccountMessages=' + res.response.result.data[i].message.message_computed_hash;
