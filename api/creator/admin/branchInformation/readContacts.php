...
// localhost/my_projects/YAMAHA_PROJECT/api/creator/admin/api/branchInformation/readEmails.php
	...
	if(!($linkstmt = $branch->displayAllContacts())){
	...
	while($linkdata = $linkstmt->fetch(PDO::FETCH_ASSOC)){
		...
		$contactKey = base64_decode($linkdata['salt']);

		$emailDataResult = array(

			'c_number' => $encrypt->decryptData($linkdata['c_number'], $contactKey),
			'c_category' => $encrypt->decryptData($linkdata['c_category'], $contactKey)
